...
// NOTE: this file is compiled INTO codonc (not the `codon` executable). The CLI
// option parsing must live in the same binary as the `cl::opt` globals it parses
// (codon/compiler/options.cpp, codon/cir/transform/numpy/numpy.cpp — both in
// codonc). On Windows, LLVM is statically linked into both codon.exe and
// codonc.dll, so each binary owns a separate `llvm::cl` CommandLine registry; if
// the driver lived in the exe its `ParseCommandLineOptions` would consult the
// exe's registry, which never sees codonc's flags ("Unknown command line
// argument" for -release/--numerics/...). Keeping the driver here guarantees a
// single registry across the exe/DLL boundary. The thin exe shim (app/main.cpp)
// forwards to codon::cliMain(); only `jupyter` mode is handled exe-side because
// it depends on codon_jupyter, not on codonc's compile flags.
...
namespace codon {
// Driver entry, exported from codonc. The `codon` executable (app/main.cpp) is a
// thin shim that handles `jupyter` itself and forwards every other invocation
// here, so that all compile-flag parsing happens in codonc's CommandLine
// registry alongside the `cl::opt` globals it must read.
int cliMain(int argc, const char **argv) {
  ...
  // `jupyter` is handled by the exe shim before reaching here.
