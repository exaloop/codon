...
void TypecheckVisitor::visit(IdExpr *expr) {
  ...
  if (expr->getType()->is(StdlibTypes::Capsule) &&
      !expr->hasAttribute(Attr::AutoGenerated)) {
    expr->setAttribute(Attr::AutoGenerated);
    resultExpr = transform(
        N<CallExpr>(N<IdExpr>(getMangledMethod("", "Capsule", "_get")), expr));
    return;
  }
...
types::FuncType *TypecheckVisitor::getThunk(types::FuncType *ft) {
  ...
  std::vector<CallArg> runtimeArgs; // do not pass generics
  for (auto &a : *ast) {
    auto arg = N<IdExpr>(a.getName());
    args.emplace_back(arg);
    if (a.isValue())
      runtimeArgs.emplace_back(clone(arg));
  }
  auto s = N<SuiteStmt>();
...
std::pair<Expr *, bool> TypecheckVisitor::getClassMember(Expr *expr,
  ...
  if (typ->is(StdlibTypes::Optional)) {
    return {transform(N<DotExpr>(
                N<CallExpr>(N<IdExpr>(getMangledMethod("", "Optional", "__unwrap__")),
                            expr),
                member)),
            true};
  }
