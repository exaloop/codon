...
public:
  AllocaInst *getPromise() const {
    ...
    return isa<ConstantPointerNull>(Arg) ? nullptr
  ...
  void setCoroutineSelf() {
    ...
    setArgOperand(CoroutineArg, ConstantExpr::getBitCast(getFunction(), Int8PtrTy));
  }
...
public:
  ...
  static bool classof(const IntrinsicInst *I) {
    auto ID = I->getIntrinsicID();
    return ID == Intrinsic::coro_id_retcon || ID == Intrinsic::coro_id_retcon_once;
  }
  static bool classof(const Value *V) {
    return isa<IntrinsicInst>(V) && classof(cast<IntrinsicInst>(V));
  }
...
class LLVM_LIBRARY_VISIBILITY CoroIdRetconOnceInst : public AnyCoroIdRetconInst {
public:
  // Methods to support type inquiry through isa, cast, and dyn_cast:
  static bool classof(const IntrinsicInst *I) {
...
public:
  AnyCoroIdInst *getId() const { return cast<AnyCoroIdInst>(getArgOperand(IdArg)); }
...
public:
  ...
  bool isFinal() const { return cast<Constant>(getArgOperand(FinalArg))->isOneValue(); }
...
public:
  ...
  Function *getMustTailCallFunction() const {
    return cast<Function>(getArgOperand(MustTailCallFuncArg)->stripPointerCasts());
  }
