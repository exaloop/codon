...
#define TOML_ENV_MESSAGE                                                               \
  ...
  "one of the 'ground truths' assumed by the library. Essentially this just means "    \
  "that I don't have the "                                                             \
  "resources to test on more platforms, but I wish I did! You can try disabling the "  \
  "checks by defining "                                                                \
  "TOML_DISABLE_ENVIRONMENT_CHECKS, but your mileage may vary. Please consider "       \
  "filing an issue at "                                                                \
...
TOML_NAMESPACE_START // abi namespace
{
  ...
  ///
  /// \detail This namespace exists so you can safely hoist the toml++ literal operators
  /// into another scope 		 without dragging in everything from the toml
  /// namespace: \cpp
  ///
  /// #include <toml++/toml.h>
  ...
  ///
  /// \remarks By default this will be an alias for std::optional, but you can change
  /// the optional type 		 used by the library by defining
  /// #TOML_OPTIONAL_TYPE.
  ...
  /// \brief	Helper class for suppressing move-construction in single-argument array
  /// constructors.
  ///
  /// \detail \cpp
  /// // desired result: [ [ 42 ] ]
...
TOML_NAMESPACE_START {
  /// \brief	Metafunction for determining if a type is, or is a reference to, a
  /// toml::table.
  template <typename T>
...
TOML_IMPL_NAMESPACE_START {
  ...
  // A: Because <algorithm> is _huge_ and std::find would be the only thing I used from
  // it.
  ...
  template <typename T>
  [[nodiscard]] TOML_ATTR(returns_nonnull) auto *make_node_specialized(T &&
                                                                       val) noexcept {
    ...
    if constexpr (is_one_of<type, array, table>) {
      ...
      static_assert(is_native<type> || is_losslessly_convertible_to_native<type>,
                    "Value initializers must be (or be promotable to) one of the TOML "
                    "value types");
...
TOML_NAMESPACE_START {
  ...
  ///			if an object of this type was inserted into a toml::table or
  /// toml::array.
  ///
  /// \note	This will return toml::node for nodes and node_views, even though a more
  /// specific node subclass
  ///			would actually be inserted. There is no way around this in a
...
TOML_NAMESPACE_START {
  ...
  /// 		 various non-conventional whitespace and newline characters, but it
  /// doesn't give 		 much thought to combining marks, grapheme clusters vs.
  /// characters, et cetera. 		 If a TOML document contains lots of codepoints
  /// outside of the ASCII range 		 you may find that your source_positions
  /// don't match those given by a text editor 		 (typically the line numbers
  /// will be accurate but column numbers will
  /// be too high). 		 <strong>This is not an error.</strong> I've chosen this
  /// behaviour as a deliberate trade-off 		 between parser complexity and
  /// correctness.
  struct TOML_TRIVIAL_ABI source_position {
    ...
    /// \brief	Returns true if two source_positions do not represent the same line and
    /// column.
    [[nodiscard]] friend constexpr bool
    operator!=(const source_position &lhs, const source_position &rhs) noexcept {
      return lhs.line != rhs.line || lhs.column != rhs.column;
    ...
    [[nodiscard]] friend constexpr bool operator<(const source_position &lhs,
                                                  const source_position &rhs) noexcept {
      return lhs.line < rhs.line || (lhs.line == rhs.line && lhs.column < rhs.column);
    }

    /// \brief	Returns true if the LHS position is before the RHS position or equal to
    /// it.
    [[nodiscard]] friend constexpr bool
    operator<=(const source_position &lhs, const source_position &rhs) noexcept {
      return lhs.line < rhs.line || (lhs.line == rhs.line && lhs.column <= rhs.column);
  ...
  struct source_region {
    ...
    ///
    /// \availability This function is only available when #TOML_WINDOWS_COMPAT is
    /// enabled.
    ///
    /// \remarks This will return an empty optional if no path was provided to
    /// toml::parse().
...
TOML_NAMESPACE_START {
  ...
  enum class format_flags : uint8_t {
    ...
    /// \brief Strings containing newlines will be emitted as triple-quoted 'multi-line'
