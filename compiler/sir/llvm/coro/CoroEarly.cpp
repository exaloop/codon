...
void Lowerer::lowerResumeOrDestroy(CallBase &CB, CoroSubFnInst::ResumeKind Index) {
...
void Lowerer::lowerCoroPromise(CoroPromiseInst *Intrin) {
  ...
  Value *Replacement = Builder.CreateConstInBoundsGEP1_32(Int8Ty, Operand, Offset);
...
void Lowerer::lowerCoroNoop(IntrinsicInst *II) {
  if (!NoopCoro) {
    ...
    Function *NoopFn = Function::Create(FnTy, GlobalValue::LinkageTypes::PrivateLinkage,
...
static bool declaresCoroEarlyIntrinsics(const Module &M) {
  return coro::declaresIntrinsics(
      M, {"llvm.coro.id", "llvm.coro.id.retcon", "llvm.coro.id.retcon.once",
          "llvm.coro.id.async", "llvm.coro.destroy", "llvm.coro.done", "llvm.coro.end",
...
struct CoroEarlyLegacy : public FunctionPass {
  ...
  StringRef getPassName() const override { return "Lower early coroutine intrinsics"; }
};
} // namespace
