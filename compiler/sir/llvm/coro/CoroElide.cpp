...
void Lowerer::elideHeapAllocations(Function *F, uint64_t FrameSize, Align FrameAlign,
                                   AAResults &AA) {
  LLVMContext &C = F->getContext();
  auto *InsertPt = getFirstNonAllocaInTheEntryBlock(CoroIds.front()->getFunction());
...
bool Lowerer::hasEscapePath(const CoroBeginInst *CB,
  ...
  do {
    ...
    if (isa<SwitchInst>(TI) && CoroSuspendSwitches.count(cast<SwitchInst>(TI))) {
...
bool Lowerer::processCoroId(CoroIdInst *CoroId, AAResults &AA, DominatorTree &DT) {
  CoroBegins.clear();
  CoroAllocs.clear();
  CoroFrees.clear();
  ResumeAddr.clear();
...
INITIALIZE_PASS_BEGIN(
    ...
    "Coroutine frame allocation elision and indirect calls replacement", false, false)
INITIALIZE_PASS_DEPENDENCY(AAResultsWrapperPass)
INITIALIZE_PASS_END(CoroElideLegacy, "coro-elide",
