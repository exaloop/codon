...
static cl::opt<bool> EnableReuseStorageInFrame(
    ...
    cl::desc("Enable the optimization which would reuse the storage in the coroutine \
...
struct SuspendCrossingInfo {
  ...
  BlockData &getBlockData(BasicBlock *BB) { return Block[Mapping.blockToIndex(BB)]; }
...
SuspendCrossingInfo::SuspendCrossingInfo(Function &F, coro::Shape &Shape) : Mapping(F) {
...
struct AllocaInfo {
  ...
  AllocaInfo(AllocaInst *Alloca, DenseMap<Instruction *, llvm::Optional<APInt>> Aliases,
             bool MayWriteBeforeCoroBegin)
...
struct FrameDataInfo {
  ...
  uint32_t getFieldIndex(Value *V) const {
    ...
    assert(Itr != FieldIndexMap.end() && "Value does not have a frame field index");
    return Itr->second;
  }
...
public:
  ...
  /// instruction.
  LLVM_NODISCARD FieldIDType addFieldForAlloca(AllocaInst *AI, bool IsHeader = false) {
...
void FrameTypeBuilder::addFieldForAllocas(const Function &F, FrameDataInfo &FrameData,
                                          coro::Shape &Shape) {
  ...
  for (const auto &A : FrameData.Allocas) {
    ...
    for (auto &AllocaSet : NonOverlapedAllocas) {
      ...
      bool NoInference = none_of(
          AllocaSet, [&](auto Iter) { return IsAllocaInferenre(Alloca, Iter); });
      ...
      bool Alignable = [&]() -> bool {
        ...
        return LargestAlloca->getAlign().value() % Alloca->getAlign().value() == 0;
      }();
...
void FrameTypeBuilder::finish(StructType *Ty) {
  ...
  for (auto &Field : Fields) {
    LayoutFields.emplace_back(&Field, Field.Size, Field.Alignment, Field.Offset);
  }
...
static StructType *buildFrameType(Function &F, coro::Shape &Shape,
  ...
  if (Shape.ABI == coro::ABI::Switch) {
    ...
    if (PromiseAlloca)
      FrameData.setFieldIndex(PromiseAlloca,
  ...
  case coro::ABI::Switch:
    ...
    Shape.SwitchLowering.IndexField = B.getLayoutFieldIndex(*SwitchIndexFieldId);
...
static Instruction *insertSpills(const FrameDataInfo &FrameData, coro::Shape &Shape) {
  ...
  auto *FramePtr = cast<Instruction>(Builder.CreateBitCast(CB, FramePtrTy, "FramePtr"));
  ...
  auto GetFramePointer = [&](Value *Orig) -> Value * {
    ...
    auto GEP =
        cast<GetElementPtrInst>(Builder.CreateInBoundsGEP(FrameTy, FramePtr, Indices));
  ...
  for (auto const &E : FrameData.Spills) {
    ...
    if (NeedToCopyArgPtrValue) {
      // For byval arguments, we need to store the pointed value in the frame,
      // instead of the pointer itself.
      auto *Value = Builder.CreateLoad(Def->getType()->getPointerElementType(), Def);
      ...
      if (CurrentBlock != U->getParent()) {
        ...
        for (DbgDeclareInst *DDI : DIs) {
          ...
          DIBuilder(*CurrentBlock->getParent()->getParent(), AllowUnresolved)
              .insertDeclare(CurrentReload, DDI->getVariable(), DDI->getExpression(),
  ...
  for (const auto &A : FrameData.Allocas) {
    ...
    if (!DIs.empty())
      DIBuilder(*Alloca->getModule(),
          ...
          .insertDeclare(G, DIs.front()->getVariable(), DIs.front()->getExpression(),
  ...
  for (const auto &A : FrameData.Allocas) {
    ...
    if (A.MayWriteBeforeCoroBegin) {
      ...
      if (Alloca->isArrayAllocation())
        report_fatal_error("Coroutines cannot handle copying of array allocas yet");
    ...
    for (const auto &Alias : A.Aliases) {
      ...
      auto *FramePtrRaw = Builder.CreateBitCast(FramePtr, Type::getInt8PtrTy(C));
      auto *AliasPtr = Builder.CreateGEP(
          FramePtrRaw, ConstantInt::get(Type::getInt64Ty(C), Alias.second.getValue()));
      auto *AliasPtrTyped = Builder.CreateBitCast(AliasPtr, Alias.first->getType());
      Alias.first->replaceUsesWithIf(AliasPtrTyped,
...
// block.
static void updatePhiNodes(BasicBlock *DestBB, BasicBlock *OldPred, BasicBlock *NewPred,
...
// PHI in InsertedBB.
static void movePHIValuesToInsertedBlock(BasicBlock *SuccBB, BasicBlock *InsertedBB,
                                         BasicBlock *PredBB,
  ...
  do {
    ...
    PHINode *InputV =
        PHINode::Create(V->getType(), 1, V->getName() + Twine(".") + SuccBB->getName(),
                        &InsertedBB->front());
...
static void rewritePHIsForCleanupPad(BasicBlock *CleanupPadBB,
  ...
  auto *UnreachBB = BasicBlock::Create(CleanupPadBB->getContext(), "unreachable",
                                       CleanupPadBB->getParent());
  ...
  auto *NewCleanupPadBB = BasicBlock::Create(
      CleanupPadBB->getContext(), CleanupPadBB->getName() + Twine(".corodispatch"),
      CleanupPadBB->getParent(), CleanupPadBB);
  Builder.SetInsertPoint(NewCleanupPadBB);
  auto *SwitchType = Builder.getInt8Ty();
  auto *SetDispatchValuePN = Builder.CreatePHI(SwitchType, pred_size(CleanupPadBB));
  CleanupPad->removeFromParent();
  CleanupPad->insertAfter(SetDispatchValuePN);
  auto *SwitchOnDispatch =
      Builder.CreateSwitch(SetDispatchValuePN, UnreachBB, pred_size(CleanupPadBB));
  ...
  for (BasicBlock *Pred : Preds) {
    ...
    auto *CaseBB =
        BasicBlock::Create(CleanupPadBB->getContext(),
                           CleanupPadBB->getName() + Twine(".from.") + Pred->getName(),
                           CleanupPadBB->getParent(), CleanupPadBB);
    updatePhiNodes(CleanupPadBB, Pred, CaseBB);
    CaseBB->setName(CleanupPadBB->getName() + Twine(".from.") + Pred->getName());
...
static void rewritePHIs(BasicBlock &BB) {
  ...
  if (auto *CleanupPad = dyn_cast_or_null<CleanupPadInst>(BB.getFirstNonPHI())) {
    SmallVector<BasicBlock *, 8> Preds(predecessors(&BB));
    for (BasicBlock *Pred : Preds) {
      if (CatchSwitchInst *CS = dyn_cast<CatchSwitchInst>(Pred->getTerminator())) {
...
static bool materializable(Instruction &V) {
  return isa<CastInst>(&V) || isa<GetElementPtrInst>(&V) || isa<BinaryOperator>(&V) ||
         isa<CmpInst>(&V) || isa<SelectInst>(&V);
}

// Check for structural coroutine intrinsics that should not be spilled into
// the coroutine frame.
static bool isCoroutineStructureIntrinsic(Instruction &I) {
  return isa<CoroIdInst>(&I) || isa<CoroSaveInst>(&I) || isa<CoroSuspendInst>(&I);
}
...
/// delete the call.
static Instruction *lowerNonLocalAlloca(CoroAllocaAllocInst *AI, coro::Shape &Shape,
                                        SmallVectorImpl<Instruction *> &DeadInsts) {
  IRBuilder<> Builder(AI);
...
static Value *emitSetSwiftErrorValue(IRBuilder<> &Builder, Value *V,
                                     coro::Shape &Shape) {
  // Make a fake function pointer as a sort of intrinsic.
  auto FnTy = FunctionType::get(V->getType()->getPointerTo(), {V->getType()}, false);
  auto Fn = ConstantPointerNull::get(FnTy->getPointerTo());
...
/// until splitting.
static Value *emitSetAndGetSwiftErrorValueAround(Instruction *Call, AllocaInst *Alloca,
                                                 coro::Shape &Shape) {
...
static void sinkSpillUsesAfterCoroBegin(Function &F, const FrameDataInfo &FrameData,
  ...
  for (auto *Def : FrameData.getAllDefs()) {
    for (User *U : Def->users()) {
      ...
      if (Inst->getParent() != CoroBegin->getParent() || Dom.dominates(CoroBegin, Inst))
        continue;
...
static void collectFrameAllocas(Function &F, coro::Shape &Shape,
  ...
  for (Instruction &I : instructions(F)) {
    ...
    // tracking.
    DominatorTree DT(F);
    AllocaUseVisitor Visitor{F.getParent()->getDataLayout(), DT, *Shape.CoroBegin,
                             Checker};
    Visitor.visitPtr(*AI);
...
void coro::salvageDebugInfo(
  ...
  DDI->setOperand(0, MetadataAsValue::get(VMContext, ValueAsMetadata::get(Storage)));
...
void coro::buildCoroutineFrame(Function &F, Shape &Shape) {
  ...
  for (Instruction &I : instructions(F)) {
    ...
    if (Shape.ABI == coro::ABI::Switch && Shape.SwitchLowering.PromiseAlloca == &I)
      continue;
