...
public:
  ...
  CoroCloner(Function &OrigF, const Twine &Suffix, coro::Shape &Shape, Kind FKind)
  ...
  CoroCloner(Function &OrigF, const Twine &Suffix, coro::Shape &Shape, Function *NewF,
...
static void maybeFreeRetconStorage(IRBuilder<> &Builder, const coro::Shape &Shape,
...
static void replaceFallthroughCoroEnd(AnyCoroEndInst *End, const coro::Shape &Shape,
  ...
  case coro::ABI::Retcon: {
    ...
    if (RetStructTy) {
      ReturnValue =
          Builder.CreateInsertValue(UndefValue::get(RetStructTy), ReturnValue, 0);
    }
...
static void createResumeEntryBlock(Function &F, coro::Shape &Shape) {
  ...
  auto *GepIndex = Builder.CreateStructGEP(FrameTy, FramePtr,
                                           Shape.getSwitchIndexField(), "index.addr");
  auto *Index = Builder.CreateLoad(Shape.getIndexType(), GepIndex, "index");
  auto *Switch = Builder.CreateSwitch(Index, UnreachBB, Shape.CoroSuspends.size());
  ...
  for (auto *AnyS : Shape.CoroSuspends) {
    ...
    if (S->isFinal()) {
      ...
      auto *GepIndex = Builder.CreateStructGEP(
          FrameTy, FramePtr, coro::Shape::SwitchFieldIndex::Resume, "ResumeFn.addr");
      auto *NullPtr = ConstantPointerNull::get(
          cast<PointerType>(cast<PointerType>(GepIndex->getType())->getElementType()));
    ...
    auto *ResumeBB = SuspendBB->splitBasicBlock(S, "resume." + Twine(SuspendIndex));
...
void CoroCloner::handleFinalSuspend() {
  assert(Shape.ABI == coro::ABI::Switch && Shape.SwitchLowering.HasFinalSuspend);
  ...
  if (isSwitchDestroyFunction()) {
    ...
    auto *GepIndex =
        Builder.CreateStructGEP(Shape.FrameTy, NewFramePtr,
                                coro::Shape::SwitchFieldIndex::Resume, "ResumeFn.addr");
    auto *Load = Builder.CreateLoad(Shape.getSwitchResumePointerType(), GepIndex);
...
static Function *createCloneDeclaration(Function &OrigF, coro::Shape &Shape,
  ...
  Function *NewF = Function::Create(FnTy, GlobalValue::LinkageTypes::InternalLinkage,
...
void CoroCloner::replaceSwiftErrorOps() { ::replaceSwiftErrorOps(*NewF, Shape, &VMap); }
...
void CoroCloner::replaceEntryBlock() {
  ...
  case coro::ABI::Switch: {
    ...
    auto *SwitchBB = cast<BasicBlock>(VMap[Shape.SwitchLowering.ResumeEntryBlock]);
  ...
  case coro::ABI::RetconOnce: {
    ...
    assert(
        (Shape.ABI == coro::ABI::Async && isa<CoroSuspendAsyncInst>(ActiveSuspend)) ||
        ((Shape.ABI == coro::ABI::Retcon || Shape.ABI == coro::ABI::RetconOnce) &&
...
Value *CoroCloner::deriveNewFramePointer() {
  ...
  case coro::ABI::Async: {
    ...
    auto *ProjectionFunc =
        cast<CoroSuspendAsyncInst>(ActiveSuspend)->getAsyncContextProjectionFunction();
    auto DbgLoc = cast<CoroSuspendAsyncInst>(VMap[ActiveSuspend])->getDebugLoc();
    // Calling i8* (i8*)
    ...
    auto *FramePtrAddr = Builder.CreateConstInBoundsGEP1_32(
        Type::getInt8Ty(Context), CallerContext, Shape.AsyncLowering.FrameOffset,
        "async.ctx.frameptr");
  ...
  case coro::ABI::RetconOnce: {
    ...
    auto FramePtrPtr = Builder.CreateBitCast(NewStorage, FramePtrTy->getPointerTo());
...
static void addFramePointerAttrs(AttributeList &Attrs, LLVMContext &Context,
                                 unsigned ParamIndex, uint64_t Size, Align Alignment) {
...
void CoroCloner::create() {
  ...
  case coro::ABI::Switch:
    ...
    addFramePointerAttrs(NewAttrs, Context, 0, Shape.FrameSize, Shape.FrameAlign);
    break;
  case coro::ABI::Async:
    break;
  case coro::ABI::Retcon:
...
static Function *createClone(Function &F, const Twine &Suffix, coro::Shape &Shape,
...
static void updateAsyncFuncPointerContextSize(coro::Shape &Shape) {
  ...
  auto *FuncPtrStruct =
      cast<ConstantStruct>(Shape.AsyncLowering.AsyncFuncPointer->getInitializer());
  auto *OrigRelativeFunOffset = FuncPtrStruct->getOperand(0);
  auto *OrigContextSize = FuncPtrStruct->getOperand(1);
  auto *NewContextSize =
      ConstantInt::get(OrigContextSize->getType(), Shape.AsyncLowering.ContextSize);
  auto *NewFuncPtrStruct = ConstantStruct::get(FuncPtrStruct->getType(),
...
static void updateCoroFrame(coro::Shape &Shape, Function *ResumeFn, Function *DestroyFn,
                            Function *CleanupFn) {
  assert(Shape.ABI == coro::ABI::Switch);

  IRBuilder<> Builder(Shape.FramePtr->getNextNode());
  auto *ResumeAddr =
      Builder.CreateStructGEP(Shape.FrameTy, Shape.FramePtr,
                              coro::Shape::SwitchFieldIndex::Resume, "resume.addr");
  ...
  auto *DestroyAddr =
      Builder.CreateStructGEP(Shape.FrameTy, Shape.FramePtr,
                              coro::Shape::SwitchFieldIndex::Destroy, "destroy.addr");
...
static void scanPHIsAndUpdateValueMap(Instruction *Prev, BasicBlock *NewBlock,
...
static bool simplifyTerminatorLeadingToRet(Instruction *InitialInst) {
  ...
  while (I->isTerminator() || (isa<CmpInst>(I) && I->getNextNode()->isTerminator())) {
    if (isa<ReturnInst>(I)) {
      if (I != InitialInst) {
...
static bool shouldBeMustTail(const CallInst &CI, const Function &F) {
  ...
  if (!CalleeParmTy->isPointerTy() || (CalleeParmTy->getPointerAddressSpace() != 0))
    return false;
...
static bool hasCallsBetween(Instruction *Save, Instruction *ResumeOrDestroy) {
  ...
  if (hasCallsInBlockBetween(ResumeOrDestroyBB->getFirstNonPHI(), ResumeOrDestroy))
    return true;
...
static void splitSwitchCoroutine(Function &F, coro::Shape &Shape,
  ...
  auto ResumeClone = createClone(F, ".resume", Shape, CoroCloner::Kind::SwitchResume);
  auto DestroyClone = createClone(F, ".destroy", Shape, CoroCloner::Kind::SwitchUnwind);
  auto CleanupClone =
...
static void coerceArguments(IRBuilder<> &Builder, FunctionType *FnTy,
  ...
  for (auto paramTy : FnTy->params()) {
    ...
    if (paramTy != FnArgs[ArgIdx]->getType())
      CallArgs.push_back(Builder.CreateBitOrPointerCast(FnArgs[ArgIdx], paramTy));
    else
...
CallInst *coro::createMustTailCall(DebugLoc Loc, Function *MustTailCallFn,
                                   ArrayRef<Value *> Arguments, IRBuilder<> &Builder) {
  auto *FnTy = cast<FunctionType>(MustTailCallFn->getType()->getPointerElementType());
...
static void splitAsyncCoroutine(Function &F, coro::Shape &Shape,
  ...
  FramePtr = Builder.CreateConstInBoundsGEP1_32(Type::getInt8Ty(Context), FramePtr,
...
static void splitRetconCoroutine(Function &F, coro::Shape &Shape,
  ...
  if (Shape.RetconLowering.IsFrameInlineInStorage) {
    ...
    RawFramePtr = Builder.CreateBitCast(RawFramePtr, Shape.CoroBegin->getType());

    // Stash the allocated frame pointer in the continuation storage.
    auto Dest =
        Builder.CreateBitCast(Id->getStorage(), RawFramePtr->getType()->getPointerTo());
  ...
  for (size_t i = 0, e = Shape.CoroSuspends.size(); i != e; ++i) {
    ...
    // Create the clone declaration.
    auto Continuation = createCloneDeclaration(F, Shape, ".resume." + Twine(i), NextF);
    Clones.push_back(Continuation);
...
static coro::Shape splitCoroutine(Function &F, SmallVectorImpl<Function *> &Clones,
                                  bool ReuseFrameSlot) {
...
static void updateCallGraphAfterCoroutineSplit(
    ...
    const SmallVectorImpl<Function *> &Clones, LazyCallGraph::SCC &C, LazyCallGraph &CG,
    CGSCCAnalysisManager &AM, CGSCCUpdateResult &UR, FunctionAnalysisManager &FAM) {
  if (!Shape.CoroBegin)
    return;
...
static void prepareForSplit(Function &F, CallGraph &CG,
  ...
  F.addFnAttr(CORO_PRESPLIT_ATTR,
              MarkForAsyncRestart ? ASYNC_RESTART_AFTER_SPLIT : PREPARED_FOR_SPLIT);
  ...
  Instruction *InsertPt = MarkForAsyncRestart
...
static void createDevirtTriggerFunc(CallGraph &CG, CallGraphSCC &SCC) {
  ...
  auto *FnTy = FunctionType::get(Type::getVoidTy(C), Type::getInt8PtrTy(C),
                                 /*isVarArg=*/false);
  Function *DevirtFn = Function::Create(FnTy, GlobalValue::LinkageTypes::PrivateLinkage,
...
static bool declaresCoroSplitIntrinsics(const Module &M) {
  return coro::declaresIntrinsics(
      M, {"llvm.coro.begin", "llvm.coro.prepare.retcon", "llvm.coro.prepare.async"});
}

static void addPrepareFunction(const Module &M, SmallVectorImpl<Function *> &Fns,
                               StringRef Name) {
...
PreservedAnalyses CoroSplitPass::run(LazyCallGraph::SCC &C, CGSCCAnalysisManager &AM,
  ...
  auto &FAM = AM.getResult<FunctionAnalysisManagerCGSCCProxy>(C, CG).getManager();
...
INITIALIZE_PASS_BEGIN(
    ...
    "Split coroutine into a set of functions driving its state machine", false, false)
INITIALIZE_PASS_DEPENDENCY(CallGraphWrapperPass)
INITIALIZE_PASS_END(CoroSplitLegacy, "coro-split",
