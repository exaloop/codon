...
coro::LowererBase::LowererBase(Module &M)
    : TheModule(M), Context(M.getContext()), Int8Ptr(Type::getInt8PtrTy(Context)),
...
Value *coro::LowererBase::makeSubFnCall(Value *Arg, int Index, Instruction *InsertPt) {
  ...
  assert(Index >= CoroSubFnInst::IndexFirst && Index < CoroSubFnInst::IndexLast &&
         "makeSubFnCall: Index value out of range");
  auto *Call = CallInst::Create(Fn, {Arg, IndexVal}, "", InsertPt);

  auto *Bitcast = new BitCastInst(Call, ResumeFnType->getPointerTo(), "", InsertPt);
  return Bitcast;
}
...
static CoroSaveInst *createCoroSave(CoroBeginInst *CoroBegin,
  ...
  auto *SaveInst = cast<CoroSaveInst>(CallInst::Create(Fn, CoroBegin, "", SuspendInst));
...
void coro::Shape::buildFrom(Function &F) {
  ...
  for (Instruction &I : instructions(F)) {
    if (auto II = dyn_cast<IntrinsicInst>(&I)) {
      ...
      case Intrinsic::coro_begin: {
        ...
        CB->removeAttribute(AttributeList::FunctionIndex, Attribute::NoDuplicate);
        CoroBegin = CB;
        break;
      }
        ...
        if (CoroEnds.back()->isFallthrough() && isa<CoroEndInst>(II)) {
          ...
          if (CoroEnds.size() > 1) {
            if (CoroEnds.front()->isFallthrough())
              report_fatal_error("Only one coro.end can be marked as fallthrough");
  ...
  case Intrinsic::coro_id_retcon_once: {
    ...
    this->ABI = (IdIntrinsic == Intrinsic::coro_id_retcon ? coro::ABI::Retcon
                                                          : coro::ABI::RetconOnce);
...
Value *coro::Shape::emitAlloc(IRBuilder<> &Builder, Value *Size, CallGraph *CG) const {
  switch (ABI) {
  ...
  case coro::ABI::RetconOnce: {
    ...
    Size = Builder.CreateIntCast(Size, Alloc->getFunctionType()->getParamType(0),
                                 /*is signed*/ false);
...
void coro::Shape::emitDealloc(IRBuilder<> &Builder, Value *Ptr, CallGraph *CG) const {
  switch (ABI) {
  case coro::ABI::Switch:
    llvm_unreachable("can't allocate memory in coro switch-lowering");
  ...
  case coro::ABI::RetconOnce: {
    ...
    Ptr = Builder.CreateBitCast(Ptr, Dealloc->getFunctionType()->getParamType(0));
...
static void checkWFRetconPrototype(const AnyCoroIdRetconInst *I, Value *V) {
  ...
  if (isa<CoroIdRetconInst>(I)) {
    ...
    if (FT->getReturnType() != I->getFunction()->getFunctionType()->getReturnType())
      fail(I,
...
void CoroAsyncEndInst::checkWellFormed() const {
  ...
  if (!MustTailCallFunc)
    return;
  auto *FnTy = cast<FunctionType>(MustTailCallFunc->getType()->getPointerElementType());
