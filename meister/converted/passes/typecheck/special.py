...
from . import access, infer, utils
...
def prepare_vtables(self: TypeVisitor):
    """
...
def generate_thunk_ast(
    ...
    derived_type = utils.instantiate_type(self.ctx, derived_generic, [base]).get_class()
    ...
    for cls_name, cls_data in self.ctx.cache.classes.items():
        ...
        for realization in list(cls_data.realizations.values()):
            ...
            if thunk_ast:
                ...
                thunk_type = utils.instantiate_type(self.ctx, thunk_fn.type).get_func()
    ...
    for tag, typ in enumerate(union.get_realization_types()):
        ...
        if is_call:
            ...
            if partial := typ.get_partial():
                ...
                matches = utils.find_matching_methods(self.ctx, typ, methods, call_args)
...
def generate_special_ast(self: TypeVisitor, function: ast.types.Function) -> ast.SuiteStmt | None:
    ...
    if (
        ...
        and utils.is_heterogenous(self.ctx, function[0])
    ):
        # Special case: do not realize auto-generated heterogenous __iter__
        raise TypecheckError(self.ctx.node_stack[-1], "expected iterable expression")
    if (
        ast_node.has(ast.Attr.AutoGenerated)
...
def transform_named_tuple(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr:
    ...
    self.prepend_stmts.append(class_suite)
...
def transform_super_f(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr:
    ...
    methods = utils.find_matching_methods(self.ctx, parent_type, supers, new_args)
    if not methods:
        raise TypecheckError(expr, "no superf methods found")
...
def transform_is_instance(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr | None:
    ...
    if (
        ...
        for base in utils.get_class(self.ctx, obj_type).mro:
            typ = utils.instantiate_type(self.ctx, base, cls)
...
def transform_has_attr(
    ...
    if exists and len(arg_types) > 1:
        ...
        exists = bool(utils.find_matching_methods(self.ctx, typ, methods, call_args))
...
def transform_tuple_fn(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr | None:
...
def transform_realized_fn(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr | None:
    ...
    if static := fn_type.get_str_static():
        ...
        if value and value.is_func():
            fn_type = utils.instantiate_type(self.ctx, value.type)
    else:
        if (
            ...
            fn_type = utils.instantiate_type(self.ctx, generalized)
...
def transform_static_tuple_type(self: TypeVisitor, expr: ast.CallExpr) -> ast.Expr | None:
    ...
    realized = infer.realize(self, utils.instantiate_type(self.ctx, fields[idx].type, [tuple_type]))
...
def generate_super_dispatch_ast(
    ...
    for descendant_name in cls_data.descendants:
        ...
        if idx < len(descendant.mro):
            next_type = utils.instantiate_type(self.ctx, descendant.mro[idx], [obj_type])
            ...
            for method in utils.find_matching_methods(self.ctx, next_type, methods, call_args):
...
def populate_static_var_types_loop(
    ...
    if union := realized.get_union():
        ...
        for field_idx, field in enumerate(utils.get_class_fields(realized)):
            field_type = utils.instantiate_type(self.ctx, field.type, [realized])
